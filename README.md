...
### find 目录名 -type d 列出所有文件夹
### find 目录名 -type f 列出所有文件
### find 目录名 -type d(f) | wc -l 计算所有目录（文件）的书目
### find 目录名|wc 查看目录下文件（包括目录等所有类型）的数量
### find 目录名|du -h 查找目录下每个目录和文件所占磁盘的多少
#
#
# cut 截取一部分内容
### -b:一个字幕一个字节 如：who|cut -b 1-3 显示第1-第3的个字母
### -c:以字符为单位分割（中文）
### -d:自定义分隔符 如：cat | grep fhl passwd | cut -d ":" -f 3,4 以：为分隔符显示第3第4部分 
### cat|grep fhl passwd 查看fhl用户信息 
#
#
# ln 在文件之间创造软连接 
...
#
#
# sudo chown -R $USER 路径/文件名 解锁文件
# chmod命令的一般格式
### chmod u/g/o/a +/-/= r/w/x/s/t/l filename
###### u 文件属主权限 g 同组用户权限 o 其他用户权限 a 所有用户权限 + 增加权限 -取消权限 = 设定权限 r 读权限 w 写权限 x 执行权限 s 文件属主和组set-ID t 粘性位* l 给文件加锁
### chmod -R 权限 目录名 给目录名下包括子目录下都设置s权限
